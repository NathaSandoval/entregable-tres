...
import { paginationLogic } from "../constants/pagination"
    ...
    <ul className="text-lg flex gap-3 justify-center flex-wrap pb-10" >
      {
        pages.map((page)=> (
          <li key={page}>
            <button className={` bg-black/20 text-blue-500 p-3 hover:bg-sky-100 hover:rounded-lg ${page === currentPage && "border-solid border-2 border-blue-500"}`} onClick={()=> setCurrentPage(page)}>
              {page}
            </button>
          </li>
        ))
